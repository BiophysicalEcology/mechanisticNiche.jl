...
# params that dont change in simulation
Ww_g = 30
alpha = 0.85
epsilon = 0.95
rho_body = 1000
...
for i in 1:length(TAs)
    global TA = TAs[i]
    global TGRD = TGRDs[i]
    global TSUBST = TGRDs[i]
    global TSKY = TSKYs[i]
    global VEL = VELs[i]
    global RH = RHs[i]
    global QSLR = QSOLRs[i]
    global Z = Zs[i]
    global K_sub = K_subs[i]

    global ZEN = Z / 180 * pi
    global RELHUM = RH # relative humidity
    global SUBTK = K_sub # substrate thermal conductivity (W/mK)
    ...
    SOLAR = qsolar(Atot=AREA,
        ...
        shade=SHADE,
        postur=postur)
    global QSOLAR = SOLAR.Qsolar


    # compute infrared radiation in
    global QIRIN = qrin(Atot=ATOT,
        Av=AV,
        At=AT,
        FAsky=FATOSK,
        FAsub=FATOSB,
    ...
    tb_new = find_zero(fun_nmr, (-50, 100), Bisection())
    push!(tbs, tb_new)
end
...
plot(tbs, label = "julia", linewidth = 2, dpi=300)
plot!(TCnmr.x, label = "NicheMapR")
ylabel!("Body temperature (ºC)")
# # savefig("sim2_vs_nmr.png")
